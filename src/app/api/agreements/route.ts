...
import { flatProgressiveTable } from "@/lib/finance";
...
export async function POST(req: NextRequest) {
  ...
  const calc = flatProgressiveTable(amount, rate, count); const due = new Date(d.dueDate); if (isNaN(due.getTime())) return fail("Vencimento inválido");
  try {
