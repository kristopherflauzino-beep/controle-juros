...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (monthlyLimit < 0) return fail("O limite mensal nao pode ser negativo");
  const client = await prisma.client.findUnique({ where: { id } }); if (!client) return fail("Cliente nao encontrado", 404);
  try {
    await prisma.$transaction([
      prisma.client.update({ where: { id }, data: { document: data.document || null, phone: data.phone || null, monthlyLimit } }),
