...
const input = z.object({ name: z.string().min(2), email: z.string().min(3), password: z.string().min(6) });
...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success) return fail("Preencha nome, login e senha com ao menos 6 caracteres");
